...
bool ofApp::checkAvailableCameras(){
    ...
    if(totalNumCameras > 0 && !camerasAvailable){
        ...
        for(uint64_t i=0; i<totalNumCameras;i++){
            ...
            camThread[i].TsetCamWidth = 1184;
            camThread[i].TsetCamHeight = 990;
            camThread[i].TxOffset = 352;
            camThread[i].TyOffset = 90;
            ...
            camThread[i].TcamZoom = camZoom;//totalNumCameras;
            camThread[i].start();
      }
        return true;
    }
...
{
    if(key == 't'){
        camThread[selectedCamera].stop();
        if(camThread[selectedCamera].TbUseTexture==true){
            camThread[selectedCamera].TbUseTexture=false;
        }else{
            camThread[selectedCamera].TbUseTexture=true;
        }
        camThread[selectedCamera].start();
    }
