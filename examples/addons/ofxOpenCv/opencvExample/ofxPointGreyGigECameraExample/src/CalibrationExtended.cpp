...
namespace ofxCvExtend {
	...
	void Calibration::setCalibFlags(int calibFlagSet){
        this->calibFlags = calibFlagSet;
	}
...
//#endif
	...
	void Calibration::draw(int i) const {
		...
		for(int j = 0; j < (int)imagePoints[i].size(); j++) {
			ofCircle(ofxCv::toOf(imagePoints[i][j]), 5);
		}
		ofPopStyle();
	}
	...
	*/
	...
	void Calibration::draw3d(int i) const {
		...
		ofxCv::applyMatrix(ofxCv::makeMatrix(boardRotations[i], boardTranslations[i]));
		...
		for(int j = 0; j < (int)objectPoints[i].size(); j++) {
			ofPushMatrix();
			ofTranslate(ofxCv::toOf(objectPoints[i][j]));
			ofCircle(0, 0, .5);
			ofPopMatrix();
		}
		...
		for(int j = 0; j < (int)objectPoints[i].size(); j++) {
			ofVec3f cur = ofxCv::toOf(objectPoints[i][j]);
			mesh.addVertex(cur);
		}
		mesh.draw();
